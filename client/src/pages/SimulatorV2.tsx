...
import { DroneSimulatorV2, type ViewMode, type DroneState, type TrajectoryPoint } from "@/lib/DroneSimulatorV2";
...
export default function SimulatorV2() {
  ...
  const simulatorRef = useRef<DroneSimulatorV2 | null>(null);
  ...
  const [pathAlgorithm, setPathAlgorithm] = useState<"astar" | "rrt">("astar");

  // 初始化 3D 场景
  useEffect(() => {
    if (!canvasRef.current) return;

    simulatorRef.current = new DroneSimulatorV2(canvasRef.current);
    ...
    if (simulatorRef.current) {
      const path = simulatorRef.current.planPath(startPos, endPos, pathAlgorithm);
      const trajectory = simulatorRef.current.generateTrajectoryFromPath(path, 15);
      
      simulatorRef.current.setTrajectory(trajectory);
      setTrajectoryPoints(trajectory);
      
      toast.success(`路径规划完成！共 ${trajectory.length} 个轨迹点`);
    }
    ...
    const interval = setInterval(() => {
      setSimulationTime((prev) => {
        ...
        if (trajectoryPoints.length > 0) {
          ...
          const nextPoint = trajectoryPoints[(trajectoryIndex + 1) % trajectoryPoints.length];
          
          // 计算方向
          const dx = nextPoint.position[0] - point.position[0];
          const dy = nextPoint.position[1] - point.position[1];
          const dz = nextPoint.position[2] - point.position[2];
          ...
          const newState: DroneState = {
            ...
            velocity: [
              point.velocity * Math.cos(newTime),
              point.velocity * Math.sin(newTime * 0.5),
              point.velocity * Math.sin(newTime),
            ],
            battery: Math.max(0, 100 - (newTime / (trajectoryPoints.length * 0.1)) * 100),
            status: "flying",
          };
  ...
  const handlePauseSimulation = () => {
    ...
    simulatorRef.current?.setDroneState({ status: isPaused ? "flying" : "paused" });
  };
  ...
  const handleReplanPath = () => {
    if (simulatorRef.current) {
      ...
      toast.success(`路径已重新规划！使用 ${pathAlgorithm.toUpperCase()} 算法`);
    }
  };
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-4 left-4 tech-frame p-4 max-w-xs space-y-4 border border-border">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button
                onClick={() => setPathAlgorithm("astar")}
                size="sm"
                variant={pathAlgorithm === "astar" ? "default" : "outline"}
                className={
                  pathAlgorithm === "astar"
                    ? "flex-1 bg-neon-pink text-background"
                    : "flex-1"
                }
              >
                A*
              ...
              <Button
                onClick={() => setPathAlgorithm("rrt")}
                size="sm"
                variant={pathAlgorithm === "rrt" ? "default" : "outline"}
                className={
                  pathAlgorithm === "rrt"
                    ? "flex-1 bg-neon-cyan text-background"
                    : "flex-1"
                }
              >
                RRT
          ...
          <div className="space-y-2">
            ...
            <Slider
              ...
              max={trajectoryPoints.length > 0 ? trajectoryPoints[trajectoryPoints.length - 1].timestamp : 100}
              step={0.01}
              className="w-full"
            />
          </div>
        ...
        <div className="absolute bottom-4 left-4 tech-frame p-4 max-w-xs space-y-2 border border-border text-xs">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-neon-cyan font-mono">
              {trajectoryPoints.length > 0
                ? (trajectoryPoints[trajectoryPoints.length - 1].timestamp).toFixed(1)
                : 0}
              s
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-muted-foreground">规划算法</span>
            <span className="text-neon-pink font-mono">
              {pathAlgorithm.toUpperCase()}
            </span>
          </div>
        </div>
        ...
        <div className="absolute bottom-4 right-4 tech-frame p-4 max-w-xs space-y-2 border border-border text-xs">
          ...
          <div className="space-y-1 text-muted-foreground">
            <div>
              <span className="text-neon-cyan">起点</span>: (-80, 5, -80)
            </div>
            <div>
              <span className="text-neon-cyan">终点</span>: (80, 30, 80)
            </div>
            <div>
              <span className="text-neon-cyan">障碍物</span>: 4 栋建筑 + 5 棵树
            </div>
            <div>
              <span className="text-neon-cyan">避障</span>: 自动规划
            </div>
          </div>
        </div>
      </div>
